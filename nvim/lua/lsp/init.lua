...
-- Ensure LSP capabilities include blink.cmp features before servers start.
do
...
local servers = {
	bash_ls = "lsp.servers.bash_ls",
	buf_ls = "lsp.servers.buf_ls",
	clangd = "lsp.servers.clangd",
	gopls = "lsp.servers.gopls",
	lua_ls = "lsp.servers.lua_ls",
	neocmakelsp = "lsp.servers.neocmakelsp",
...
if vim.lsp.config then
	for name, module in pairs(servers) do
		...
		if ok and type(config) == "table" and next(config) ~= nil then
...
vim.api.nvim_create_autocmd({ "LspAttach" }, {
	...
	callback = function(event)
		...
		map("gt", tb.lsp_type_definitions, "Type [D]efinition")
		...
		if client and client:supports_method(vim.lsp.protocol.Methods.textDocument_codeLens, event.buf) then
			...
			vim.keymap.set("n", "<leader>cl", vim.lsp.codelens.run, { buffer = event.buf, desc = "LSP: Code Lens Run" })
		end
